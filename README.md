...
- **Task** – 과제  
      ...
      1. Nori 사용자 user_dictionary, 품사 필터 적용 인덱스 생성 및 시 텍스트 분석
      2. 실습: 오픈뱅킹 데이터를 활용한 대시보드 구현

- **Action** – 행동

      1. Elasticsearch 인덱스 생성
      2. Nori 토크나이저 커스터마이징, 품사 기반 불용어 처리, 한자 처리
      3. 구두점 유지, 특정 품사 제거, 숫자 처리
      4. KQL, Lucene 쿼리와 Query DSL 호환 사용

- **Result** – 결과  

      1. 시 전문을 해당 분석기로 토큰화 및 분석
      2. 키바나를 사용한 대시보드 만들기
